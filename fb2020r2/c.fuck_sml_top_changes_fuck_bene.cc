...
void solve() {
  ...
  auto upd = [&](int eid, ll neww) {
    if (eid < n*m) { // small circles
      ...
      if (x[i]!=y[i]) {
        ...
        max_cut2_bene = max(max_cut2_bene, ll(sml_bet[i].top() + sml_oth[i].top() - sml[i].top()));
      }
    } else {
  ...
  auto calc = [&]() {
    ...
    if (max_cut2_bene >=0 ) {
      ll a2 = sum_sml_top + max_cut2_bene;
      a = max(a, a2);
    }
    a = sw - a;
    return (a%P + P)%P;
